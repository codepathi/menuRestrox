...
const Carousel: React.FunctionComponent<CarouselPropsType> = ({
  ...
  let nextButtonClasses = cn(
    'w-7 h-7 md:w-8 md:h-8 lg:w-9 lg:h-9 xl:w-10 xl:h-10 text-base lg:text-lg xl:text-xl cursor-pointer flex items-center justify-center rounded-full bg-skin-fill absolute transition duration-300 hover:bg-skin-primary hover:text-skin-inverted focus:outline-none transform shadow-navigation dark:bg-darkBg',
  ...
  return (
    ...
    >
      ...
      {Boolean(navigation) && (
        <div
          className={`flex items-center w-full absolute top-16 z-10 ${buttonGroupClassName}`}
        >
          ...
          {nextActivateId.length > 0 ? (
            <div className={nextButtonClasses} id={nextActivateId}>
              {dir === 'rtl' ? (
                <IoIosArrowBack className="dark:text-darkText" />
              ) : (
                <IoIosArrowForward className="dark:text-darkText" />
              )}
            </div>
          ) : (
